...
- Billing: 100 free auths then 1 EUR/100 successful auths (blocks; PORTIER_BLOCK/FREE_AUTHS tunable). `billing='exempt'` (set via `portier app-billing`) is counted but never charged and never blocked — comped apps; operator-only, no HTTP surface. Best-effort charge, never blocks in-flight login; past_due blocks only NEW initiations. Merchant key via PEAGE_MERCHANT_KEY env var.
...
- Deploy: see /etc/portier/portier.env on the host for production config (PEAGE_MERCHANT_KEY, PORTIER_KEK, PORTIER_SECRET). Canonical host: portier.intrane.fr.
